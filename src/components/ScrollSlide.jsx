...
export default function ScrollSlide() {
  ...
  const trackWidth = 24; // matches w-6 (24px) in Tailwind
  const trackHeightV = 70; // 70vh height of the slider
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Spawn splash particles at the scroll indicator's position
  ...
  useEffect(() => {
    ...
    const runParticleLoop = () => {
      ...
      // Spawn a bubble every 12 frames at the monkey's position
      if (frameCounter.current % 12 === 0) {
        particlesRef.current.push({
          type: "bubble",
          x: indicatorX + (Math.random() - 0.5) * 6, // slight scatter
          y: indicatorY + 20, // spawn at the monkey's base
          vy: -(Math.random() * 0.7 + 0.4), // float up slowly
          size: Math.random() * 2.5 + 1.2, // bubble size
      ...
      particlesRef.current = particlesRef.current.filter((p) => {
        if (p.type === "bubble") {
          p.y += p.vy; // move up
          p.x += Math.sin(Date.now() * p.swaySpeed + p.swayOffset) * 0.12; // gentle sway
          
          // Pop if bubble reaches top of track
  ...
  return (
    <div
      ref={containerRef}
      className="fixed top-[15vh] right-4 w-6 h-[70vh] z-50 rounded-full border border-cyan-400/20 bg-[#E3F6F7]/25 backdrop-blur-xs shadow-md shadow-cyan-500/5 flex flex-col items-center overflow-hidden hidden md:block"
    >
      {/* SVG Water Slide Path */}
      ...
      {/* Splash & Bubble Particle Canvas */}
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none z-10" />

      {/* Monkey/King Kong Slide Image */}
      <motion.div
